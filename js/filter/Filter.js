...
form.addEventListener("submit", async (e) => {
  e.preventDefault();
  cards.innerHTML = ""; //Pour évité que le loader ne se centre en dessous du viewport on réinitialisé la liste des cards

  // HIDDEN MODAL
  modal.style.visibility = "hidden";
  form.classList.toggle("modal");

  const jobsSubmit = await showJobsSearch( form.fullTimeFilter.checked ? 1 : 0, form.companyFilter.value, form.locationFilter.value );
...
searchBtn.addEventListener("click", async () => {
  ...
  const jobsSearch = await showJobsSearch( form.fullTimeFilter.checked ? 1 : 0, form.companyFilter.value );
